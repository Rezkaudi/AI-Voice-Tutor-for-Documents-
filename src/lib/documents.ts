...
import type { DocumentChunk, DocumentPage, UploadKind } from "./types";
...
export function detectUploadKind(file: UploadDescriptor): UploadKind | null {
  ...
  if ((extension === "md" || extension === "markdown") || MARKDOWN_TYPES.has(mimeType)) {
    return "markdown";
  }
...
export function validateUploadFile(file: UploadDescriptor): ValidationResult {
  ...
  if (!kind) {
    return { ok: false, error: "This MVP supports searchable PDFs, .txt, and .md files." };
  }
...
export function chunkDocumentPages(
  pages: (Pick<DocumentPage, "pageNumber" | "text"> & Partial<Pick<DocumentPage, "documentId">>)[],
  ...
  for (const page of pages) {
    ...
    const segments = splitTextWithOverlap(normalized, maxChars, overlapChars);
...
function splitTextWithOverlap(text: string, maxChars: number, overlapChars: number): string[] {
  ...
  while (start < text.length) {
    ...
    const end = roughEnd === text.length ? roughEnd : findSoftBoundary(text, start, roughEnd);
...
function makeSnippet(text: string): string {
  ...
  return normalized.length <= 240 ? normalized : `${normalized.slice(0, 237).trim()}...`;
}

function extensionOf(fileName: string): string {
  const lastPart = fileName.toLowerCase().split(".").pop();
  return lastPart === fileName.toLowerCase() ? "" : lastPart ?? "";
}
