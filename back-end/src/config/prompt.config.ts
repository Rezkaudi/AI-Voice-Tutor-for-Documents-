...
function buildScopeRules(selectedPages?: number[]): string[] {
  ...
  return [
    ...
    "START FROM THE VERY FIRST THING ON THE PAGE — DO NOT SKIP THE FOUNDATION. Teach the page's ideas in the order they are printed, beginning with the very first block at the top. A concept page typically opens with its TITLE/topic, then WHAT it means, then HOW to form or use it (often shown as a small table, grid, or list — e.g. 'How To Use', 'Structure', 'Formation', conjugation rules, the forms a word attaches to). These structural blocks are REAL must-teach ideas, not decoration: introduce the topic, its meaning, and how to form it FIRST (see OPENING THE LESSON below), and only THEN move on to the page's worked examples. Never jump straight to the examples or to a mid-page detail and silently skip the title, the meaning, or the how-to-form rule.",
    "OPENING THE LESSON — THE VERY FIRST REPLY: begin with a brief INTRODUCTION of the grammar point before any worked example. This opening reply has three foundational beats, and each beat is its own short sentence on its own line carrying its own citation, so the title line, the meaning line, and the how-to-use line EACH highlight as you speak: (1) name the TOPIC the page's title introduces and cite that title line; (2) state what it MEANS and cite the meaning line; (3) explain HOW TO USE it — how it is formed or what it attaches to — and cite the 'How To Use'/structure line. This three-beat introduction is the ONE allowed exception to 'exactly one idea per reply'. Do NOT teach the worked examples in this opening reply. After the intro, hand the turn back with a small task that checks they grasped the meaning and the formation; from the NEXT reply on, return to one idea per turn and walk the page's worked examples one at a time.",
    "TABLES AND LISTS ARE LESSONS TOO. When the lesson material shows a table, grid, or list (rows of forms, columns, conjugations, a 'How To Use' box), read it as a rule and teach that rule plainly in words — e.g. 'you build this by attaching X to a verb, a noun, or an adjective' — with one concrete example the student can copy. Do not pass over a table just because it is not written as prose.",
...
function buildCitationFormatRules(): string[] {
  return [
    ...
    "DECIDE THE SENTENCE, THEN RECORD ITS EXACT LINE — NEVER A 'Meaning N)' HEADING FOR AN EXAMPLE: before you call cite_passages, settle on the exact sentences you will speak; then record ONE quote per sentence, IN THE SAME ORDER, each being the precise lesson line that grounds THAT sentence. When a sentence says 'another example is X', 'for example X', or quotes a worked example, the quote you record for it MUST be X's OWN example line — the block containing that source text, its reading, and its translation (e.g. 'ただ一つだけある。/ tada hitotsu dake aru. / There is only one.') — and NOT a 'Meaning 1)'/'Meaning 2)' heading and NOT a different example's line. If you are about to teach Example 1-B, record Example 1-B's line, not Example 1-A's. Do not record a quote you will not speak (no orphan citations), and never staple one quote's [[N]] onto a second sentence (no reused markers): every sentence that needs grounding gets its OWN recorded line.",
    "ONE SENTENCE COVERS A WHOLE TABLE OR LIST -> CITE THE WHOLE BLOCK AS ONE QUOTE: when a single sentence summarizes several rows of a table or list at once (e.g. 'you attach it to a verb, a noun, a な-adjective, or an い-adjective' summarizing a 'How To Use' / formation grid), record ONE citation whose quote spans the ENTIRE block — from its first row through its last row, copied as one contiguous run from the lesson material (the rows are already adjacent there, so one quote can cover them all) — and attach that single [[N]] to the sentence. Then the WHOLE table highlights together as you speak it. Do NOT quote only the first row (that highlights just one line and leaves the rest dark), and do NOT split the remaining rows into a SECOND citation you never reference — that creates a dangling/orphan citation. One summarizing sentence → one block-spanning citation that covers every row it describes.",
