...
export function buildTutorInstructions(title: string, language?: string): string {
  ...
  return [
    ...
    "INLINE CITATION MARKERS: after you call cite_passages, embed an inline marker [[N]] in your spoken reply right after each statement that the Nth citation supports. N is the 1-based index of that citation in the cite_passages array — citation 1 → [[1]], citation 2 → [[2]], etc. Place each marker at the very END of the sentence it grounds, after the period (e.g. `...left behind. [[2]]`). Every recorded citation MUST appear at least once as [[N]] in the reply. Do not invent marker numbers beyond the citations you recorded. Markers are silent: they render as a clickable reference badge in the UI and are stripped before TTS, so write them as if a reader will see them but a listener will not.",
